...
// EXTERNAL MODULE: ./node_modules/.pnpm/@earendil-works+pi-coding-agent@0.74.0_ws@8.19.0_zod@4.3.6/node_modules/@earendil-works/pi-coding-agent/dist/index.js + 210 modules
var dist = __webpack_require__(44862);
// EXTERNAL MODULE: ./node_modules/.pnpm/@earendil-works+pi-ai@0.74.0_ws@8.19.0_zod@4.3.6/node_modules/@earendil-works/pi-ai/dist/index.js + 7 modules
var pi_ai_dist = __webpack_require__(67605);
...
var usage = __webpack_require__(72746);
;// CONCATENATED MODULE: ./src/sdk/runtimes/model-selectors.ts
/**
...
function parseModelSelector(model) {
    if (!isPiModelSelector(model)) {
        throw new Error(`Pi model selector must be provider/model: ${model}. For example openai/gpt-5.5.`);
    }
    const slashIndex = model.indexOf('/');
    return {
...
function createAuthStorage(model, legacyAnthropicApiKey) {
    const authStorage = dist.AuthStorage.create();
...
async function runPiPrompt(options) {
    ...
    const modelRegistry = dist.ModelRegistry.create(authStorage);
...
function toStructuredPrompt(kind, task, schema) {
    const jsonSchema = external.toJSONSchema(schema);
    return [
...
function toPiCustomTools(tools, executeTool) {
    ...
    return tools.map((tool) => (0,dist.defineTool)({
        ...
        parameters: pi_ai_dist.Type.Unsafe(tool.inputSchema),
